...
public class CameraController : MonoBehaviour
{
    ...
    {
        var centerPos = new Vector3(((sizeX - 1) / 2f) * BoardManager.DOTS_DISTANCE, ((sizeY - 1) / 2f) * BoardManager.DOTS_DISTANCE, transform.position.z);
        transform.position = centerPos;
        limitPosition = new Vector3(sizeX * BoardManager.DOTS_DISTANCE, sizeY * BoardManager.DOTS_DISTANCE, transform.position.z);

        // Adjust the camera's orthographic size based on the size of the board
        Camera.main.orthographicSize = Mathf.Max(sizeX, sizeY) * BoardManager.DOTS_DISTANCE / 2;
